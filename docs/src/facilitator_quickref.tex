...
\section{Phase 1 --- Threat Model and Placement}
...
\item \textbf{Set the condition first} if you are collecting data. It locks the
moment any team acts.
\item A fresh session \textbf{loads Scenario 1 automatically}. For another, pick
it from the dropdown and press \textbf{Load}. Loading sets the organization,
read-aloud text and budget, locks the threat model, and moves play to Phase 1.
\item Set \textbf{Teams} to about class size $\div$ 3, rounded up.
\item Copy the player and projector links from \textbf{Links to share}.
\item Read the scenario aloud. Do not paraphrase --- teams comparing notes later
need to have heard the same words.
\end{enumerate}
...
\toprule
\textbf{Solutions} & Measures list, then buy controls on a budget \\
\textbf{Control} & Measures list only. No cards, no budget \\
\bottomrule
\end{tabularx}
\normalsize
\vspace{0.3em}

Everything else --- guidance, timing, phases, interface --- is identical. Both arms
list the measures they would recommend, \textbf{including what they cannot
afford}; that is what makes breadth comparable between them.

\vspace{0.3em}
Separate sessions per arm:\\
\texttt{?session=fall-a}\\
\texttt{?session=fall-b\&condition=control}

\section{During play}
\begin{itemize}
\item \textbf{Deal once} at Phase 2. Dealing again replaces every hand.
\item \textbf{Timer} --- one button starts the right length for the current phase.
Everyone sees it, not just the projector.
\item \textbf{Adjudication} --- justifications queue with an amber badge.
\emph{Specific enough} or \emph{Return to deck}. One prompt only.
\item \textbf{Coverage stays hidden until Phase 3}, so teams cannot shop by
watching what lights up.
\item \textbf{Accessibility} --- text size and light/dark sit in the header. Say
so at the start; students rarely find them and some will not ask.
\end{itemize}
...
\textbf{Treating the two arms differently.} If you explain more to a control team
because they look stuck, you have compared a guided session with an unguided one
and the study means nothing. Same scenario reading, same intervention rules, same
manner. The software holds the interface constant; you hold yourself constant.
\end{redbox}

\vspace{0.4em}
\begin{greenbox}{If the app misbehaves}
The build version sits in grey beside \emph{How to play}. If a student's differs
from yours, they have a cached page --- hard reload. A blank screen now shows a
readable error instead; have them read the line to you.
\vspace{0.3em}

If it fails outright, keep playing on paper. Every mechanic works with the
physical deck, a printed $3\times3$ grid and six coins. Collect the record sheets

...
\subsection{Phase 1 --- Threat Model and Placement (5--7 minutes)}
...
Teams first list the measures they would recommend, including any they cannot
afford, then purchase a portfolio. For each purchase they must say which
safeguard addresses which element of the threat. The list and the purchases are
different questions --- what the organization needs, and what this budget buys.
...
\textbf{Coverage decides movement.} At each engagement the portfolio is scored
against the scenario's expert key: strong cards count two, partial cards one. A
score of two or more and the threat becomes \emph{less} likely; a score of one
and it holds; a score of zero and it drifts upward. Deferred Investment adds one
cell on top, so a team that covers well and defers stays level rather than
improving --- that is what the extra points cost. Controls already bought stay
installed and are not dealt again, and every engagement brings a fresh budget, so
a team that spent everything in round one is never stranded.
...
what the team purchased in earlier engagements. In the web version this is
automatic --- press \emph{Next engagement} rather than \emph{New round}, and the
app carries threats forward, applies drift to teams that deferred, resolves
realization against everything they have ever bought, and grants the banked
ten-point budget. The resolution below is what it applies, and what you apply by
hand on paper:
